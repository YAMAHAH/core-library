...
namespace ApiService.Middleware
{
    ...
    public class TokenProviderMiddleware
    {
        ...
        public async Task GenerateToken(HttpContext context)
        {
            ...
            var usersign = context.Request.Form["usersign"]; //JsonService.SerializeObject(new { username = "TEST" })


            var identity = await _options.IdentityResolver(username, usersign);
            if (identity == null)
            {
                context.Response.StatusCode = 400;
