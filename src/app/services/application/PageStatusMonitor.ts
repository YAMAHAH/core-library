...
import { STATUS_ACTIVE, STATUS_IDLE, STATUS_HIDDEN } from './StatusConsts';
...
export class PageStatusMonitor {
    ...
    private root = typeof self === "object" && self.self === self && self ||
    ...
    on(event: string, callback: (data) => any): PageStatusMonitor {
        Events.attach(event, callback);
        Events.subscribe(event, callback);
        return this;
    }

    off(event: string, callback?: any): PageStatusMonitor {
        Events.remove(event, callback);
        Events.unsubscribe(event, callback);
        return this;
    }

    setIdleDuration(seconds: number): PageStatusMonitor {
    ...
    idle(callback?: (data: any) => any): PageStatusMonitor {
        if (callback) {
            ...
            Events.publish("idle");
            Events.publish('statusChanged', [{ status: this.status }]);
        }
        return this;
    }

    blur(callback?: (data: any) => any): PageStatusMonitor {
        if (callback) {
    ...
    focus(callback?: (data: any) => any): PageStatusMonitor {
        if (callback) {
            ...
            Events.fire("wakeup");
            Events.fire("statusChanged", [{ status: this.status }]);
            Events.publish("focus");
            Events.publish("wakeup");
            Events.publish("statusChanged", [{ status: this.status }]);
        }
        return this;
    }

    wakeup(callback?: (data: any) => any): PageStatusMonitor {
        if (callback) {
